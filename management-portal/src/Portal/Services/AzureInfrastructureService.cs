...
namespace Stamps.ManagementPortal.Services
{
    ...
    public class DiscoveredResourceGroup
    {
    ...
    {
        public static ResourceGroupType Classify(string rgName)
        {
    ...
    public class AzureInfrastructureService : IAzureInfrastructureService
    {
        ...
        public async Task<InfrastructureDiscoveryResult> DiscoverInfrastructureAsync()
        {
            ...
            var discoveredResourceGroups = new List<DiscoveredResourceGroup>();
            ...
            await foreach (var subscription in subscriptions)
                {
                    try
                    {
                        ...
                        await foreach (var resourceGroup in resourceGroups)
                        {
                            try
                            {
                                // Classify resource group
                                var rgType = ResourceGroupClassifier.Classify(resourceGroup.Data.Name);
                                discoveredResourceGroups.Add(new DiscoveredResourceGroup
                                {
                                    Name = resourceGroup.Data.Name,
                                    Type = rgType
                                });

                                // Only treat as CELL if classified as Cell
                                if (rgType == ResourceGroupType.Cell)
                                {
                ...
                return new InfrastructureDiscoveryResult
                {
                    ...
                    DiscoveredResourceGroups = discoveredResourceGroups
                };
            }
    ...
    public class InfrastructureDiscoveryResult
    {
        ...
        public List<DiscoveredResourceGroup> DiscoveredResourceGroups { get; set; } = new();
    }
}
